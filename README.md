...
生成专辑大纲前可填写每集最少和最多字数，默认 2000–2500。系统按汉字和数字逐字计数、连续英文单词计 1 字，忽略标点、Markdown 标记和空白。该范围会锁定在后台任务中并传给专辑大纲、声音细纲、初稿和终稿；初稿或终稿超出范围时自动调整最多两次，仍不合格才记录失败并允许单条重跑。
...
系统角色、Markdown 输出结构、章节标识、来源边界、数量修复和字数修复仍由后端保护，不能在页面中删除。专辑创作阶段不输出严格 JSON；内部小输入步骤只把已完成 Markdown 转换为页面数据。提示词修改只影响新任务；运行中的单条或批量任务继续使用启动时锁定的提示词版本。
