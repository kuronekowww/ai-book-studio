...
专辑大纲提示词会围绕听众关心的问题组织选题，标题灵活使用冲突、悬念、反常识或因果追问；每集主要内容包含“听众钩子”“核心主题”和两至四条“核心要点”，同时继续保留知识资产 ID 与原文索引。
...
页面右侧列出当前环节可用的占位符。专辑大纲必须保留 `{{book_analysis}}`；声音细纲必须保留 `{{episode_framework}}` 和 `{{source_text}}`；初稿必须保留 `{{episode_outline}}` 和 `{{source_text}}`；终稿必须保留 `{{episode_draft}}` 和 `{{source_text}}`。保存前会检查缺失、未知或花括号不完整的占位符。

系统角色、JSON 输出结构、知识资产与原文索引约束仍由后端保护，不能在页面中删除。预览只填充占位符，不调用模型。提示词修改只影响新任务；运行中的单条或批量任务继续使用启动时锁定的提示词版本。
