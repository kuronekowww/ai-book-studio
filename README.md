...
页面右侧列出当前环节可用的占位符。专辑大纲必须至少保留 `{{chapter_catalog}}`、`{{module_source}}` 或兼容旧版本的 `{{book_analysis}}` 之一；声音细纲必须保留 `{{episode_framework}}` 和 `{{source_text}}`；初稿必须保留 `{{episode_outline}}` 和 `{{source_text}}`；终稿必须保留 `{{episode_draft}}` 和 `{{source_text}}`。保存前会检查缺失、未知或花括号不完整的占位符。

四个环节都可使用 `{{episode_word_count_range}}`。即使旧的项目级模板尚未加入该占位符，系统保护层也会注入项目字数范围，并覆盖模板中“约 1500 字”等旧篇幅要求。

系统角色、Markdown 输出结构和章节标识约束仍由后端保护，不能在页面中删除。专辑创作阶段不输出严格 JSON；内部小输入步骤只把已完成 Markdown 转换为页面数据。预览只填充占位符，不调用模型。提示词修改只影响新任务；运行中的单条或批量任务继续使用启动时锁定的提示词版本。
